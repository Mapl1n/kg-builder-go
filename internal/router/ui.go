...
<div class="header">
  ...
  <div style="display:flex;gap:8px;align-items:center">
    <input id="searchInput" placeholder="Search entity..." style="width:200px" onkeydown="if(event.key==='Enter')searchEntity()">
...
<div class="container">
  <div class="sidebar">
    <div class="upload-zone" id="dropZone" onclick="document.getElementById('fileInput').click()">
      <p style="font-size:28px">PDF</p><p>Upload PDF/DOCX/TXT</p>
      <p style="font-size:11px;color:var(--muted);margin-top:4px">Auto NER + relation extraction</p>
    </div>
    ...
    <h3>Entities</h3><div id="entityList" style="max-height:300px;overflow-y:auto"><p style="font-size:12px;color:var(--muted)">Upload a document to extract entities</p></div>
    <h3 style="margin-top:16px">Relations</h3><div id="relationList" style="max-height:200px;overflow-y:auto"><p style="font-size:12px;color:var(--muted)">Relations will appear here</p></div>
  </div>
...
var typeNames={'person':'Person','org':'Organization','contract':'Contract','date':'Date','money':'Money','location':'Location'};
...
function showPlaceholder(){
  chart.setOption({
    ...
    graphic:{type:'text',left:'center',top:'center',style:{text:'Upload a document\nto build knowledge graph',font:'16px sans-serif',fill:'#64748b',textAlign:'center'}}
  });
}
showPlaceholder();

document.getElementById('dropZone').ondragover=e=>{e.preventDefault()};
document.getElementById('dropZone').ondrop=e=>{e.preventDefault();var f=e.dataTransfer.files[0];if(f)doUpload(f)};
...
async function doUpload(file){
  ...
  try{
    ...
    if(d.code===0){
      ...
      toast('Extracted '+(d.data.entity_count)+' entities, '+(d.data.relation_count)+' relations','success');
      renderEntities(allEntities);
      renderRelations(allRelations);
      if(allEntities.length>0){
        setTimeout(function(){focusEntity(allEntities[0].id,allEntities[0].name);},300);
      }
...
function renderEntities(entities){
  document.getElementById('entityList').innerHTML=entities.map(function(e){
    return '<div class="entity-item" onclick="focusEntity(\''+e.id+'\',\''+e.name+'\')" style="'+(currentFocus===e.id?'border-color:var(--accent)':'')+'"><span class="type-badge" style="background:'+(typeColors[e.type]||'#666')+'">'+(typeNames[e.type]||e.type)+'</span><span style="font-size:13px">'+e.name+'</span><br><span style="font-size:11px;color:var(--muted)">Doc: '+e.doc_name+'</span></div>';
  }).join('');
}
...
function updateGraph(entities,relations){
  currentGraph={
    nodes:entities.map(function(e){return{id:e.id,name:e.name,type:e.type,symbolSize:30,category:entityGroup[e.type]||0,itemStyle:{color:typeColors[e.type]||'#666'}}}),
    edges:relations.map(function(r){return{source:r.subject,target:r.object,label:{show:true,formatter:r.predicate,fontSize:10}}})
  };
  renderGraph();
}

var categories=Object.keys(typeNames).map(function(k,i){return{name:typeNames[k],itemStyle:{color:Object.values(typeColors)[i]}}});
var entityGroup={person:0,org:1,contract:2,date:3,money:4,location:5};

function renderGraph(){
  chart.setOption({
    backgroundColor:'transparent',
    tooltip:{formatter:function(p){return p.dataType==='node'?p.name+'<br>'+typeNames[p.data.type]:p.name}},
...
async function searchEntity(){
  ...
  try{
    ...
    }else{toast('No entity found','error')}
  }catch(e){toast(e.message,'error')}
}
...
async function focusEntity(entityID,name){
  ...
  try{
    ...
    if(d.code===0&&d.data){
      ...
      renderGraph();
      renderEntities(allEntities);
      toast('Focus: '+name+' (nodes:'+currentGraph.nodes.length+', edges:'+currentGraph.edges.length+')','success');
    }
  }catch(e){toast(e.message,'error')}
}
